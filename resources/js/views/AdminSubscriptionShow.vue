<template>
  <div>
    <div id="layout-dashboard">
      ...
      <h1 class="title-primary">{{this.content['organizations'].name}}</h1>
      <header class="subscription-header">
        <div class="status-container">
          <div :class="`status status-${this.content['organizations'].subscriptions[0].status.type}`" @click.prevent="openActions">
        <span
          class="text-subhead"
        >{{this.content['organizations'].subscriptions[0].status.admin_label}}</span>
      </div>
      <div class="actions-container action-status">
         <form
                class="form-update-status"
                @submit.prevent="changeStatus(1)"
              >
                <button
      ...
      <div class="card-row">
        <div class="card-info card-col">
          ...
          <span class="text-body-display">{{this.content['organizations'].subscriptions[0].total_dancer}}</span>
        </div>
        <div class="card-info card-col">
            <h2 class="title-headline">{{ $t('dashboard.table.title.routine') }}</h2>
            <span class="text-body-display">{{this.content['organizations'].subscriptions[0].routines_count}}</span>
        </div>
        <div class="card-info card-col">
          <h2 class="title-headline">{{ $t('dashboard.title.debt') }}</h2>
          <span class="text-body-display">{{this.content['organizations'].subscriptions[0].balance}} $</span>
        </div>
      </div>
      </header>
      ...
      <div class="tabs">
        <div class="tab active" data-tab="inscriptions">
          ...
          <section
            ...
            :set="subscription = this.content['organizations'].subscriptions[0]"
          >
            <h1 class="title-tertiary">{{ $t('admin.title.routines') }}</h1>
            <div class="alert alert-no-data" v-if="!subscription.routines.length">
              <p class="alert-text text-body-display">{{ $t('admin.text.noRoutines') }}</p>
            </div>
...
>
                ...
                >
                  ...
                  <li class="table-item grid-2">
                    ...
                    <div class="actions-container">
                      <router-link
                        :to="{ name: 'admin.routineEdit', params: {id: routine.subscription_id, routine_id: routine.id} }"
                      ...
                      <router-link
                        :to="{ name: 'admin.routineDuplicate', params: {id: routine.subscription_id, routine_id: routine.id} }"
                        class="action table-action"
                      >
            ...
            <div class="export-actions">
              ...
              >Exporter</a>
              <a class="btn btn-primary btn-inverted" @click.prevent="clickAddToSchedule">Ajout à l'ordre</a>
              <router-link
                :to="{ name: 'admin.routineCreate', params: {event: this.event_name, id: this.content['organizations'].subscriptions[0].id, organization_id: this.content['organizations'].id} }"
                class="btn btn-primary"
              >
                <span class="text-subhead">{{ $t('forms.actions.addRoutine') }}</span>
              </router-link>
            </div>
          </section>
          <section
            ...
            :set="subscription = this.content['organizations'].subscriptions[0]"
          >
            <h1 class="title-tertiary">{{ $t('admin.title.invoice') }}</h1>
            <div class="alert alert-no-data" v-if="!subscription.routines.length">
              <p class="alert-text text-body-display">{{ $t('admin.text.noInvoice') }}</p>
            </div>
            <div class="table" v-if="subscription.routines.length">
              <div class="table-header">
                <ul class="table-list">
                  ...
                  <li class="table-item grid-2">
                    <span class="text-subhead">{{ $t('dashboard.table.title.routine') }}</span>
                  </li>
                  <li class="table-item grid-2">
                    <span class="text-subhead">{{ $t('dashboard.table.title.totalSubscription') }}</span>
                  </li>
            ...
            <admin-fee :fees="subscription.fees" :event="this.event_name" :subscription_id="this.subscription_id" :feeTypes="this.content['feeTypes']" />

            <ul class="invoice-list-total">
              <li class="invoice-item text-body-display">
                <span class="invoice-data grid-4">Sous-total</span>
                <span class="invoice-int grid-3">{{subscription.sub_total}} $</span>
              </li>
              <li class="invoice-item text-body-display" v-if="subscription.event.state_id == 57">
                <span class="invoice-data grid-4">TPS 737664490 RT 0001 (5%)</span>
                <span class="invoice-int grid-3">{{subscription.tps}} $</span>
              </li>
              <li class="invoice-item text-body-display" v-if="subscription.event.state_id == 57">
                <span class="invoice-data grid-4">TVQ 1224260896 TQ 0001 (9,975%)</span>
                <span class="invoice-int grid-3">{{subscription.tvq}} $</span>
              </li>
              <li class="invoice-item text-body-display" v-if="subscription.event.state_id == 58">
                <span class="invoice-data grid-4">TVH 737664490 RT 0001 (13%)</span>
                <span class="invoice-int grid-3">{{subscription.tvh}} $</span>
              </li>
              <li class="invoice-item invoice-total text-body-display">
                <span class="invoice-data grid-4">{{ $t('admin.label.total_cost') }}</span>
                <span class="invoice-int grid-3">{{subscription.total}} $</span>
              </li>
            </ul>
            <div class="export-actions">
              <a class="btn btn-primary btn-inverted" :href=authUrl target="_blank">{{ $t('admin.actions.generate.invoice') }}</a>
              <a class="btn btn-primary" @click.prevent="showFeeModal($event)" target="_blank">{{ $t('admin.actions.add.fee') }}</a>
              <form
                class="form-update-status"
                @submit.prevent="changeStatus(5)"
                v-if="this.content['organizations'].subscriptions[0].status_id == 2"
              >
                <button
                  class="btn btn-primary btn-change-status btn-waiting"
                  type="submit"
                  :disabled="saving"
                >{{ $t('admin.actions.add.waiting') }}</button>
              </form>
              <form
                class="form-update-status"
                @submit.prevent="changeStatus(3)"
                v-if="this.content['organizations'].subscriptions[0].status_id == 1 || this.content['organizations'].subscriptions[0].status_id == 2 || this.content['organizations'].subscriptions[0].status_id == 5"
              >
                <button
          ...
          <section
            ...
            :set="subscription = this.content['organizations'].subscriptions[0]"
          >
            <h1 class="title-tertiary">{{ $t('admin.title.payments') }}</h1>
            <div class="alert alert-no-data" v-if="!subscription.payments.length">
              <p class="alert-text text-body-display">{{ $t('admin.text.noPayments') }}</p>
            </div>
            <div class="table" v-else>
              <div class="table-header">
                <ul class="table-list">
                  <li class="table-item grid-2">
                    <span class="text-subhead">{{ $t('dashboard.table.title.date') }}</span>
                  </li>
                  <li class="table-item grid-2">
                    <span class="text-subhead">{{ $t('dashboard.table.title.type') }}</span>
                  </li>
                  <li class="table-item grid-6">
                    <span class="text-subhead">{{ $t('dashboard.table.title.note') }}</span>
                  </li>
                  <li class="table-item grid-2">
                    <span class="text-subhead">{{ $t('dashboard.table.title.amount') }}</span>
                  </li>
                </ul>
              </div>
              <div class="table-body">
                ...
                >
                  <li class="table-item grid-2">
                    <span class="table-text text-body-display">{{payment.receive_on}}</span>
                  </li>
                  <li class="table-item grid-2">
                    <span class="table-text text-body-display">{{payment.payment_type.name}}</span>
                  </li>
                  <li class="table-item grid-6">
                    <span class="table-text text-body-display">{{payment.note}}</span>
                  </li>
                  <li class="table-item grid-2">
                    <span class="table-text text-body-display">{{payment.formatted_amount}} $</span>
                    <div class="table-menu" @click.prevent="openActions">
                      <icon icon="menu" class></icon>
                    </div>
                    <div class="actions-container">
            ...
            <ul class="invoice-list-total">
              ...
              <li class="invoice-item text-body-display">
                <span class="invoice-data grid-4">{{ $t('admin.text.totalToPay') }}</span>
                <span class="invoice-int grid-3">{{subscription.total}} $</span>
              </li>
              <li class="invoice-item text-body-display">
                <span class="invoice-data grid-4">{{ $t('admin.text.totalPayments') }}</span>
                <span class="invoice-int grid-3">{{subscription.sum_payments}} $</span>
              </li>
              <li class="invoice-item invoice-total text-body-display">
                <span class="invoice-data grid-4">{{ $t('admin.text.balance') }}</span>
                <span class="invoice-int grid-3">{{subscription.balance}} $</span>
              </li>
            </ul>
            <div class="export-actions">
              <button
              ...
              >{{ $t('admin.actions.add.payment') }}</button>
            </div>
          </section>
        </div>
                  ...
                  <div class="form-group" v-bind:class="{'has-error' : errors.has('name')}">
                    <div class="floating-label-container">
                      <input
                        ...
                        v-bind:class="{'has-value' : this.organization.user.name}"
                        type="text"
                        :disabled="true"
                        v-on:change="isEmpty"
                  ...
                  <div class="form-group" v-bind:class="{'has-error' : errors.has('email')}">
                    <div class="floating-label-container">
                      <input
                        id="user_email"
                        class="form-text-field-input"
                        v-bind:class="{'has-value' : this.user.email}"
                        type="email"
                        v-on:change="isEmpty"
                        :disabled="true"
          ...
          <div class="tab-col grid-4">
            ...
            >
              <section class="form-step" data-step="1">
                <div class="form-group" v-bind:class="{'has-error' : errors.has('name')}">
                  <div class="floating-label-container">
                    <input
                      ...
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.name}"
                      type="text"
                      v-on:change="isEmpty"
                ...
                <div class="form-group" v-bind:class="{'has-error' : errors.has('accronyme')}">
                  <div class="floating-label-container">
                    <input
                      id="organization_accronyme"
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.accronyme}"
                      type="text"
                      v-on:change="isEmpty"
                  ...
                  <p v-show="errors.has('accronyme')" class="form-msg-error">{{ errors.first('accronyme') }}</p>
                </div>
                <div class="form-group" v-bind:class="{'has-error' : errors.has('address')}">
                  <div class="floating-label-container">
                    <input
                      ...
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.address}"
                      type="text"
                      v-on:change="isEmpty"
                ...
                <div class="form-group" v-bind:class="{'has-error' : errors.has('city')}">
                  <div class="floating-label-container">
                    <input
                      ...
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.city}"
                      type="text"
                      v-on:change="isEmpty"
                ...
                <div class="form-group" v-bind:class="{'has-error' : errors.has('zipcode')}">
                  <div class="floating-label-container">
                    <input
                      ...
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.zipcode}"
                      type="text"
                      v-on:change="isEmpty"
                ...
                <div class="form-group" v-bind:class="{'has-error' : errors.has('phone')}">
                  <div class="floating-label-container">
                    <input
                      ...
                      class="form-text-field-input"
                      v-bind:class="{'has-value' : this.organization.phone}"
                      type="text"
                      v-on:change="isEmpty"
        ...
        <div class="tab tab-dancers" data-tab="dancers">
          <section class="dancer-container" :set="dancers = this.content['organizations'].dancers">
              ...
              <div class="table-header">
                <ul class="table-list">
                  <li class="table-item grid-3">
                    <span class="text-subhead">{{ $t('dashboard.table.title.firstName') }}</span>
                  </li>
                  <li class="table-item grid-3">
                    <span class="text-subhead">{{ $t('dashboard.table.title.lastName') }}</span>
                  </li>
                  <li class="table-item grid-3">
                    <span class="text-subhead">{{ $t('dashboard.table.title.birthday') }}</span>
                  </li>
                  <li class="table-item grid-3">
                    <span class="text-subhead">{{ $t('dashboard.table.title.age') }}</span>
                  </li>
                </ul>
              </div>
              <div class="table-body">
                ...
                >
                  <li class="table-item grid-3">
                    <span class="table-text text-body-display">{{dancer.first_name}}</span>
                  </li>
                  <li class="table-item grid-3">
                    <span class="table-text text-body-display">{{dancer.last_name}}</span>
                  </li>
                  <li class="table-item grid-3">
                    <span class="table-text text-body-display">{{dancer.date_of_birth}}</span>
                  </li>
                  <li class="table-item grid-3">
            ...
            <section class="dancer-actions">
              <button
                class="btn btn-primary"
                @click.prevent="addDancer"
              >{{$t('dashboard.label.add.dancer')}}</button>
            </section>
          </section>
        </div>
      </div>
    </div>
    <modal name="payment" :classes="'modal'" height="auto" @before-open="beforeOpenPayment">
      <article class="modal-inner">
        <header class="modal-header">
          <h1 class="modal-title title-primary">{{$t('forms.title.add.payment')}}</h1>
        ...
        <div class="modal-body">
          ...
          >
            <section class="form-step" data-step="1">
              <div class="form-group" v-bind:class="{'has-error' : errors.has('receive_on')}">
                <div class="floating-label-container">
                  <cleave
                    ...
                    class="form-text-field-input"
                    v-bind:class="{'has-value' : this.payment.receive_on}"
                    name="receive_on"
                    v-on:change.native="isEmpty"
                  ></cleave>
                  <label
                    class="floating-label"
                    for="payment_receive_on"
                  >{{ $t('forms.label.receive_on') }}</label>
                </div>
                <p
                  v-show="errors.has('receive_on')"
                  class="form-msg-error"
                >{{ errors.first('receive_on') }}</p>
                <p class="form-hint text-caption">AAAA/MM/JJ</p>
              </div>
              <div class="form-group" v-bind:class="{'has-error' : errors.has('payment_type_id')}">
                <div class="floating-label-container">
                  <select
                    id="payment_type"
                  ...
                  >
                    <option
                      v-for="option in this.content['paymentTypes']"
                      v-bind:key="option.id"
                      v-bind:value="option.id"
                    >{{option.name}}</option>
                  </select>
                  <label class="floating-label" for="payment_type">{{ $t('forms.label.type') }}</label>
                </div>
              </div>
              <div class="form-group" v-bind:class="{'has-error' : errors.has('amount')}">
                <div class="floating-label-container">
                  <input
                    id="payment_amount"
                    class="form-text-field-input"
                    v-bind:class="{'has-value' : this.payment.amount}"
                    type="text"
                    v-on:change="isEmpty"
                    v-model="payment.amount"
                  />
                  <label class="floating-label" for="payment_amount">{{ $t('forms.label.amount') }}</label>
                </div>
                <p v-show="errors.has('amount')" class="form-msg-error">{{ errors.first('amount') }}</p>
              </div>
              <div class="form-group" v-bind:class="{'has-error' : errors.has('note')}">
                <div class="floating-label-container">
                  <input
                    id="payment_note"
                    class="form-text-field-input"
                    v-bind:class="{'has-value' : this.payment.note}"
                    type="text"
                    v-on:change="isEmpty"
                    v-model="payment.note"
                  />
                  <label class="floating-label" for="payment_note">{{ $t('forms.label.note') }}</label>
                </div>
                <p v-show="errors.has('note')" class="form-msg-error">{{ errors.first('note') }}</p>
              </div>
              <div class="form-actions">
                <div
    ...
    <modal name="dancer" :classes="'modal'" height="auto" @before-open="beforeOpenDancer">
      <article class="modal-inner">
        <header class="modal-header">
