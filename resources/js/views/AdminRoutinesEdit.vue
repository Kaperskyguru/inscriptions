<template>
  <div>
    <div id="layout-dashboard">
      <feedback></feedback>
      <h1 class="title-primary">{{ $t('dashboard.title.routinesEdit') }}</h1>
  ...
  methods: {
    ...
    onSubmitDancer(ev) {
      this.storeDancer({
        ...
        organization_id: this.routineData.routine.organization_id
      })
        .then(() => {
    ...
    onSubmit(ev) {
      ...
      if (this.route_name === "admin.routineDuplicate") {
        this.store({
